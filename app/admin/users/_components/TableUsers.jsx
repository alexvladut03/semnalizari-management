...
export default async function TableUsers() {
  ...
  return (
    <div className="border-2 border-gray-500 rounded-lg">
      <div className="grid grid-cols-4 bg-gray-900/80 rounded-t-lg p-2 w-full">
      ...
      {users.map((user, index) => {
        ...
        return (
          <div
            key={user.id}
            className={`grid grid-cols-4 items-center bg-gray-600/50 p-4 border-t-2 border-gray-500 ${
              isLast ? "rounded-b-lg" : ""
            }`}
          >
            <span className="text-white text-sm">{user.id}</span>
            <span className="text-white">{user.username}</span>
            <span className="text-white truncate">{user.password}</span>
